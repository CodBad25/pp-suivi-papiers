  ...
  return (
    <div style={{ padding: 16, maxWidth: 1200, margin: '0 auto', fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0, color: '#1f2937' }}>📄 Gestion des Documents</h1>
      ...
      <div style={{ marginBottom: 20, background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 8, padding: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          ...
          <button 
            ...
            style={{ padding: '8px 12px', borderRadius: 6, border: '2px solid #8b5cf6', background:'#8b5cf6', color:'#fff', fontWeight:600 }}
          >
            Créer
          </button>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {periodes.map((p:any)=> (
            <div key={p.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', border:'1px solid #e5e7eb', borderRadius:6, padding:'8px 12px', background: selectedPeriode?.id===p.id?'#f3e8ff':'#fff' }}>
              {editingPeriode?.id === p.id && editingPeriode ? (
                <div style={{ display:'flex', flex:1, gap:8, alignItems:'center' }}>
                  <input 
                    ...
                    style={{ flex:1, height:32, padding:'0 8px', border:'2px solid #8b5cf6', borderRadius:6, outline:'none' }}
                    autoFocus
                    onKeyDown={(e)=>{
                      if(e.key==='Enter'){
                ...
                <>
                  <button 
                    ...
                    style={{ 
                      background: selectedPeriode?.id===p.id ? '#8b5cf6' : '#f8fafc', 
                      border: `2px solid ${selectedPeriode?.id===p.id ? '#8b5cf6' : '#e2e8f0'}`, 
                      borderRadius: 8,
                      padding: '8px 12px',
                      fontSize: 14, 
                      ...
                      boxShadow: selectedPeriode?.id===p.id ? '0 2px 4px rgba(139, 92, 246, 0.3)' : '0 1px 2px rgba(0, 0, 0, 0.05)'
                    }}
                    onMouseEnter={(e) => {
          ...
          {periodes.map((p: any) => (
            <button 
              ...
              style={{ 
                padding: '8px 16px', 
                borderRadius: 6, 
                border: `2px solid ${selectedPeriode?.id === p.id ? '#8b5cf6' : '#d1d5db'}`, 
                background: selectedPeriode?.id === p.id ? '#f3e8ff' : '#fff',
                color: selectedPeriode?.id === p.id ? '#8b5cf6' : '#374151',
      ...
      {selectedPeriode && (
        <div style={{ background: '#fff', border: '2px solid #8b5cf6', borderRadius: 12, padding: 20 }}>
          <h3 style={{ marginTop: 0, marginBottom: 16, fontSize: 20, fontWeight: 600, color: '#1f2937' }}>Documents pour la période "{selectedPeriode.name}"</h3>
                          ...
                          if (e.key === 'Enter') {
                            if (editingDoc.name.trim() && editingDoc.name.trim() !== d.name) {
                              fetch(`/api/documents/${d.id}`, {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ name: editingDoc.name.trim() })
                              }).then(() => reload());
                            }
                            setEditingDoc(null);
                          }
                    ...
                    <>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button 
                          onClick={async () => {
                            if (!inP) {
                              await fetch(`/api/periodes/${selectedPeriode.id}/document-types`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ documentTypeId: d.id })
                              });
                            } else {
                              await fetch(`/api/periodes/${selectedPeriode.id}/document-types?documentTypeId=${d.id}`, {
                                method: 'DELETE'
                              });
                            }
                          ...
                          style={{ 
                            padding: '6px 12px', 
                            borderRadius: 6, 
                            border: `2px solid ${inP ? '#ef4444' : '#8b5cf6'}`, 
                            background: inP ? '#fff' : '#8b5cf6', 
                        ...
                        <button 
                          onClick={() => setEditingDoc({ id: d.id, name: d.name })} 
                          style={{ 
                            padding: '6px 12px', 
                            borderRadius: 6, 
                        ...
                        {deletingDoc?.id === d.id ? (
                          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                            <span style={{ fontSize: 12, color: '#ef4444', fontWeight: 600 }}>Supprimer "{d.name}" ?</span>
                            <button 
                              onClick={async () => {
