  ...
  return (
    <div style={{ padding: 16, maxWidth: 1200, margin: '0 auto', fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, sans-serif' }}>
      ...
      <div style={{ display:'flex', gap:12, flexWrap:'wrap', marginBottom: 16 }}>
        {periodes.map((p:any)=> (
          <button key={p.id} onClick={()=>setSelectedPeriode(p)} style={{ padding:'6px 10px', borderRadius: 6, border: `2px solid ${selectedPeriode?.id===p.id?'#3b82f6':'#d1d5db'}`, background: selectedPeriode?.id===p.id?'#eff6ff':'#fff', cursor:'pointer' }}>{p.name}</button>
        ))}
      </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
          ...
          <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:8, padding:12 }}>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:8 }}>
              <input value={newTaskName} onChange={(e)=>setNewTaskName(e.target.value)} placeholder="Nouvelle tâche" style={{ flex:1, height:32, padding:'0 8px', border:'2px solid #d1d5db', borderRadius:6 }} />
          ...
          <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:8, padding:12 }}>
            ...
            }} style={{ padding:'8px 12px', borderRadius:6, border:'2px solid #16a34a', background:'#16a34a', color:'#fff', fontWeight:600 }}>Activer la période</button>
          </div>
        </div>
      )}
    </div>
